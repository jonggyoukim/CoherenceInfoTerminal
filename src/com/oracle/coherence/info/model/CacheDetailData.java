...
public class CacheDetailData extends AbstractData {
	...
	public List<Map.Entry<Object, Data>> getJMXData(MBeanServerConnection server, VisualVMModel model) {
		...
		if (model.getSelectedCache() != null) {
			...
			try {
				Set<ObjectName> resultSet = server.queryNames(
						new ObjectName("Coherence:type=Cache,service=" + selectedCache.getX() + ",name=" + selectedCache.getY() + ",*"), null);
				...
				for (Iterator<ObjectName> iter = resultSet.iterator(); iter.hasNext();) {
					...
					if (objName.getKeyProperty("tier").equals("back")) {
						...
						data.setColumn(CacheDetailData.CACHE_MISSES, (Long) server.getAttribute(objName, "CacheMisses"));
						...
						data.setColumn(CacheDetailData.HIT_PROBABILITY, (Double) server.getAttribute(objName, "HitProbability"));
