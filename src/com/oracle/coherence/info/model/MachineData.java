...
public class MachineData extends AbstractData {
	...
	public List<Map.Entry<Object, Data>> getJMXData(MBeanServerConnection server, VisualVMModel model) {
		...
		try {
			// loop through each of the entries and get the data for a given
			// node only
			// as we know that nodes on the same machine will have the same
			// results
			...
			while (iter.hasNext()) {
				...
				String sQuery = "Coherence:type=Platform,Domain=java.lang,subType=OperatingSystem,nodeId=" + entry.getKey().getY() + ",*";
				...
				for (Iterator<ObjectName> iterResults = resultSet.iterator(); iterResults.hasNext();) {
					...
					data.setColumn(MachineData.FREE_PHYSICAL_MEMORY, (Long) server.getAttribute(objectName, "FreePhysicalMemorySize"));
					data.setColumn(MachineData.SYSTEM_LOAD_AVERAGE, (Double) server.getAttribute(objectName, "SystemLoadAverage"));
					data.setColumn(MachineData.PROCESSOR_COUNT, (Integer) server.getAttribute(objectName, "AvailableProcessors"));
					data.setColumn(MachineData.TOTAL_PHYSICAL_MEMORY, (Long) server.getAttribute(objectName, "TotalPhysicalMemorySize"));

					data.setColumn(MachineData.PERCENT_FREE_MEMORY,
							((Long) data.getColumn(MachineData.FREE_PHYSICAL_MEMORY) * 1.0f) / (Long) data.getColumn(MachineData.TOTAL_PHYSICAL_MEMORY));
