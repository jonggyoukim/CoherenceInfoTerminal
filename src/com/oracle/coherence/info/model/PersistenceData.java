...
import com.oracle.coherence.info.helper.JMXUtils;
import com.oracle.coherence.info.helper.VisualVMModel;
import com.oracle.coherence.info.helper.JMXUtils.Attribute;

/**
 * A class to hold persistence data. <strong>Note:</strong> This data is
 * experimental and may be inaccurate and may change in a future release.
 *
	...
	public List<Map.Entry<Object, Data>> getJMXData(MBeanServerConnection server, VisualVMModel model) {
		...
		try {
			...
			for (Iterator<ObjectName> serviceNameIter = servicesSet.iterator(); serviceNameIter.hasNext();) {
				...
				if (!"n/a".equals(sPersistenceMode)) {
					...
					if (data == null) {
						...
						// get the number of snapshots from the
						// PersistenceManager MBean
						String[] asSnapshots = (String[]) JMXUtils.runJMXQuerySingleResult(server, "Coherence:type=PersistenceSnapshot,service=" + serviceName
								+ ",responsibility=SnapshotManager", new Attribute("Snapshots"));
			...
			for (Iterator<Object> serviceNameIter = mapData.keySet().iterator(); serviceNameIter.hasNext();) {
				...
				// select only the current service so we can determine the
				// number of storage-enabled
				// members.
				Set<ObjectName> resultSet = server.queryNames(new ObjectName("Coherence:type=Service,name=" + serviceName + ",*"), null);
				...
				for (Iterator<ObjectName> iter = resultSet.iterator(); iter.hasNext();) {
					...
					if ((Boolean) server.getAttribute(objectName, "StorageEnabled")) {
						...
						// PersistenceActiveSpaceUsed is only valid for active
						// persistence mode
						if ("active".equals(data.getColumn(PersistenceData.PERSISTENCE_MODE))) {
							data.setColumn(PersistenceData.TOTAL_ACTIVE_SPACE_USED, (Long) data.getColumn(PersistenceData.TOTAL_ACTIVE_SPACE_USED)
						...
						// count the nodes for working out the average at the
						// end as we are adding up
						// all of the averages, putting them in the average
						// latency
						// and average them
						cNodes++;
						data.setColumn(PersistenceData.AVERAGE_LATENCY,
								(Float) data.getColumn(PersistenceData.AVERAGE_LATENCY) + (Float) server.getAttribute(objectName, "PersistenceLatencyAverage"));
				...
				if (cNodes > 0) {
					data.setColumn(PersistenceData.AVERAGE_LATENCY, (Float) data.getColumn(PersistenceData.AVERAGE_LATENCY) / cNodes);
				}

				data.setColumn(PersistenceData.TOTAL_ACTIVE_SPACE_USED_MB, (Long) data.getColumn(PersistenceData.TOTAL_ACTIVE_SPACE_USED) / 1024);
