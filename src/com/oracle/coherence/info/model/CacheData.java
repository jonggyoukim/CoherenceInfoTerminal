...
public class CacheData extends AbstractData {
	...
	public List<Map.Entry<Object, Data>> getJMXData(MBeanServerConnection server, VisualVMModel model) {
		...
		try {
			...
			// loop through each cache and find all the different node entries
			// for the caches
			...
			for (Iterator<Object> cacheNameIter = mapData.keySet().iterator(); cacheNameIter.hasNext();) {
				...
				Set resultSet = server.queryNames(new ObjectName("Coherence:type=Cache,service=" + sServiceName + ",name=" + sCacheName + ",*"), null);

				boolean fisSizeCounted = false; // indicates if non dist cache
												// size has been counted
				...
				for (Iterator iter = resultSet.iterator(); iter.hasNext();) {
					...
					if (objectName.getKeyProperty("tier").equals("back")) {
						...
						if (fIsDistributedCache || (!fIsDistributedCache && !fisSizeCounted)) {
							data.setColumn(CacheData.SIZE, (Integer) data.getColumn(CacheData.SIZE) + (Integer) server.getAttribute(objectName, "Size"));
						...
						// memory is always going to be counted, even for
						// replicated caches
						data.setColumn(CacheData.MEMORY_USAGE_MB,
								(Integer) data.getColumn(CacheData.MEMORY_USAGE_MB) + (Integer) server.getAttribute(objectName, "Units"));

						data.setColumn(CacheData.MEMORY_USAGE_BYTES,
								(Long) data.getColumn(CacheData.MEMORY_USAGE_BYTES) + (Integer) server.getAttribute(objectName, "Units"));
