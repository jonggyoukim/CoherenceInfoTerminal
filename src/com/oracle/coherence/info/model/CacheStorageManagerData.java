...
public class CacheStorageManagerData extends AbstractData {
	...
	public static final int INDEX_INFO = 8;
	...
	public List<Map.Entry<Object, Data>> getJMXData(MBeanServerConnection server, VisualVMModel model) {
		...
		if (model.getSelectedCache() != null) {
			...
			try {
				Set<ObjectName> resultSet = server.queryNames(new ObjectName("Coherence:type=StorageManager,service=" + selectedCache.getX() + ",cache="
						+ selectedCache.getY() + ",*"), null);
				for (Iterator<ObjectName> iter = resultSet.iterator(); iter.hasNext();) {
					...
					data.setColumn(CacheStorageManagerData.LOCKS_GRANTED, (Integer) server.getAttribute(objName, "LocksGranted"));
					data.setColumn(CacheStorageManagerData.LOCKS_PENDING, (Integer) server.getAttribute(objName, "LocksPending"));
					data.setColumn(CacheStorageManagerData.LISTENER_REGISTRATIONS, (Long) server.getAttribute(objName, "ListenerRegistrations"));
					data.setColumn(CacheStorageManagerData.MAX_QUERY_DURATION, (Long) server.getAttribute(objName, "MaxQueryDurationMillis"));
					data.setColumn(CacheStorageManagerData.MAX_QUERY_DESCRIPTION, (String) server.getAttribute(objName, "MaxQueryDescription"));
					data.setColumn(CacheStorageManagerData.NON_OPTIMIZED_QUERY_AVG, (Long) server.getAttribute(objName, "NonOptimizedQueryAverageMillis"));
					data.setColumn(CacheStorageManagerData.OPTIMIZED_QUERY_AVG, (Long) server.getAttribute(objName, "OptimizedQueryAverageMillis"));
					data.setColumn(CacheStorageManagerData.INDEX_INFO, (String[]) server.getAttribute(objName, "IndexInfo"));
