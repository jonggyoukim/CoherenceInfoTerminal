...
public class ServiceMemberData extends AbstractData {
	...
	public List<Map.Entry<Object, Data>> getJMXData(MBeanServerConnection server, VisualVMModel model) {
		...
		if (sSelectedService != null) {
			...
			try {
				Set<ObjectName> servicesSet = server.queryNames(new ObjectName("Coherence:type=Service,name=" + sSelectedService + ",*"), null);
				...
				for (Iterator<ObjectName> serviceNameIter = servicesSet.iterator(); serviceNameIter.hasNext();) {
					...
					data.setColumn(ServiceMemberData.TASK_BACKLOG, ((Integer) server.getAttribute(serviceNameObjName, "TaskBacklog")).intValue());
					data.setColumn(ServiceMemberData.THREAD_COUNT, ((Integer) server.getAttribute(serviceNameObjName, "ThreadCount")).intValue());
					data.setColumn(ServiceMemberData.THREAD_IDLE_COUNT, ((Integer) server.getAttribute(serviceNameObjName, "ThreadIdleCount")).intValue());
					data.setColumn(ServiceMemberData.REQUEST_AVERAGE_DURATION,
							((Float) server.getAttribute(serviceNameObjName, "RequestAverageDuration")).floatValue());
					data.setColumn(ServiceMemberData.TASK_AVERAGE_DURATION,
							((Float) server.getAttribute(serviceNameObjName, "TaskAverageDuration")).floatValue());

					if ((Integer) data.getColumn(ServiceMemberData.THREAD_COUNT) > 1) {
						float threadUtil = ((float) ((Integer) data.getColumn(ServiceMemberData.THREAD_COUNT) - (Integer) data
								.getColumn(ServiceMemberData.THREAD_IDLE_COUNT))) / (Integer) data.getColumn(ServiceMemberData.THREAD_COUNT);
