	...
	public List<Map.Entry<Object, Data>> getJMXData(MBeanServerConnection server, VisualVMModel model) {
		...
		try {
			...
			for (Iterator<ObjectName> nodIter = proxyNamesSet.iterator(); nodIter.hasNext();) {
				...
				// only include the NameService if the model tells us we should
				// and
				...
				if (("NameService".equals(sServiceName) && model.isIncludeNameService()) || !"NameService".equals(sServiceName)) {
					...
					data.setColumn(ProxyData.CONNECTION_COUNT, (Integer) server.getAttribute(proxyNameObjName, "ConnectionCount"));
					data.setColumn(ProxyData.OUTGOING_MSG_BACKLOG, (Long) server.getAttribute(proxyNameObjName, "OutgoingMessageBacklog"));
					data.setColumn(ProxyData.TOTAL_BYTES_RECEIVED, (Long) server.getAttribute(proxyNameObjName, "TotalBytesReceived"));
					data.setColumn(ProxyData.TOTAL_BYTES_SENT, (Long) server.getAttribute(proxyNameObjName, "TotalBytesSent"));
					data.setColumn(ProxyData.TOTAL_MSG_RECEIVED, (Long) server.getAttribute(proxyNameObjName, "TotalMessagesReceived"));
					data.setColumn(ProxyData.TOTAL_MSG_SENT, (Long) server.getAttribute(proxyNameObjName, "TotalMessagesSent"));
