	...
	public List<Map.Entry<Object, Data>> getJMXData(MBeanServerConnection server, VisualVMModel model) {
		...
		try {
			...
			for (Iterator<ObjectName> nodIter = setNodeNames.iterator(); nodIter.hasNext();) {
				...
				data.setColumn(MemberData.PUBLISHER_SUCCESS, (Float) server.getAttribute(nodeNameObjName, "PublisherSuccessRate"));
				data.setColumn(MemberData.RECEIVER_SUCCESS, (Float) server.getAttribute(nodeNameObjName, "ReceiverSuccessRate"));
				data.setColumn(MemberData.MAX_MEMORY, (Integer) server.getAttribute(nodeNameObjName, "MemoryMaxMB"));
				data.setColumn(MemberData.FREE_MEMORY, (Integer) server.getAttribute(nodeNameObjName, "MemoryAvailableMB"));
				data.setColumn(MemberData.SENDQ_SIZE, (Integer) server.getAttribute(nodeNameObjName, "SendQueueSize"));

				data.setColumn(MemberData.USED_MEMORY, (Integer) data.getColumn(MemberData.MAX_MEMORY) - (Integer) data.getColumn(MemberData.FREE_MEMORY));
