	...
	public List<Map.Entry<Object, Data>> getJMXData(MBeanServerConnection server, VisualVMModel model) {
		...
		try {
			...
			for (Iterator<ObjectName> cacheNameIter = servicesSet.iterator(); cacheNameIter.hasNext();) {
				...
				// if its dist cache then save so we dont, double count size for
				// repl
				// caches
			...
			for (Iterator<Object> cacheNameIter = mapData.keySet().iterator(); cacheNameIter.hasNext();) {
				...
				// members.
				Set resultSet = server.queryNames(new ObjectName("Coherence:type=Service,name=" + sServiceName + ",*"), null);
				...
				for (Iterator iter = resultSet.iterator(); iter.hasNext();) {
					...
					// only update the static information once as it will be the
					// same across all members
					if (data.getColumn(ServiceData.PARTITION_COUNT) == null) {
						data.setColumn(ServiceData.PARTITIONS_ENDANGERED, ((Integer) server.getAttribute(objName, "PartitionsEndangered")).intValue());
						data.setColumn(ServiceData.PARTITIONS_UNBALANCED, ((Integer) server.getAttribute(objName, "PartitionsUnbalanced")).intValue());
						data.setColumn(ServiceData.PARTITIONS_VULNERABLE, ((Integer) server.getAttribute(objName, "PartitionsVulnerable")).intValue());
						data.setColumn(ServiceData.STATUS_HA, (String) server.getAttribute(objName, "StatusHA"));
						data.setColumn(ServiceData.PARTITIONS_PENDING, (int) ((Long) server.getAttribute(objName, "RequestPendingCount")).longValue());
						data.setColumn(ServiceData.PARTITION_COUNT, ((Integer) server.getAttribute(objName, "PartitionsAll")).intValue());
					}
					...
					if (((Boolean) server.getAttribute(objName, "StorageEnabled")).booleanValue()) {
						data.setColumn(ServiceData.STORAGE_MEMBERS, (Integer) data.getColumn(ServiceData.STORAGE_MEMBERS) + 1);
					}
