...
public class HttpSessionData extends AbstractData {
	 ...
	 * {@inheritDoc }
	 */
	public List<Map.Entry<Object, Data>> getJMXData(MBeanServerConnection server, VisualVMModel model) {
		...
		try {
			...
			Set<ObjectName> applicationSet = server.queryNames(new ObjectName("Coherence:type=WebLogicHttpSessionManager,*"), null);
			...
			for (Iterator<ObjectName> applicationIter = applicationSet.iterator(); applicationIter.hasNext();) {
				...
				data.setColumn(HttpSessionData.SESSION_TIMEOUT, (Integer) server.getAttribute(objName, "SessionTimeout"));
				...
				// populate the session sizes by obtaining the
				// "SessionCacheName" and querying this
			...
			// loop through the individual entries and query the detail
			// information
			String sQueryPredicate = isWebLogicServer ? "Coherence:type=WebLogicHttpSessionManager" : "Coherence:type=HttpSessionManager";
			...
			for (Iterator<Object> iter = mapData.keySet().iterator(); iter.hasNext();) {
				...
				Set<ObjectName> resultSet = server.queryNames(new ObjectName(sQueryPredicate + ",appId=" + sAppId + ",*"), null);
				...
				for (Iterator<ObjectName> iterDetail = resultSet.iterator(); iterDetail.hasNext();) {
					...
					if (nCurrentLastReapDuration > ((Long) data.getColumn(HttpSessionData.LAST_REAP_DURATION_MAX)).longValue()) {
					...
					data.setColumn(HttpSessionData.SESSION_UPDATES,
							(Integer) data.getColumn(HttpSessionData.SESSION_UPDATES) + (Integer) server.getAttribute(objName, "SessionUpdates"));

					data.setColumn(HttpSessionData.AVG_SESSION_SIZE,
							(Integer) data.getColumn(HttpSessionData.AVG_SESSION_SIZE) + (Integer) server.getAttribute(objName, "SessionAverageSize"));
					data.setColumn(HttpSessionData.AVG_REAP_DURATION,
							(Long) data.getColumn(HttpSessionData.AVG_REAP_DURATION) + (Long) server.getAttribute(objName, "AverageReapDuration"));
					data.setColumn(HttpSessionData.AVG_REAPED_SESSIONS,
							(Long) data.getColumn(HttpSessionData.AVG_REAPED_SESSIONS) + (Long) server.getAttribute(objName, "AverageReapedSessions"));
					data.setColumn(HttpSessionData.TOTAL_REAPED_SESSIONS,
							(Long) data.getColumn(HttpSessionData.TOTAL_REAPED_SESSIONS) + (Long) server.getAttribute(objName, "ReapedSessions"));
				...
				if (c > 1) {
					data.setColumn(HttpSessionData.AVG_SESSION_SIZE, (Integer) data.getColumn(HttpSessionData.AVG_SESSION_SIZE) / c);
					data.setColumn(HttpSessionData.AVG_REAP_DURATION, (Long) data.getColumn(HttpSessionData.AVG_REAP_DURATION) / c);
					data.setColumn(HttpSessionData.AVG_REAPED_SESSIONS, (Long) data.getColumn(HttpSessionData.AVG_REAPED_SESSIONS) / c);
				}
			}
	...
	/**
	 * Return the count of objects in the given cache name. If the cache is in
	 * multiple services, which should not be the case for Coherence*Web, the
	 * count will be the first entry. The reason is that there is no service
	 * name storage against the MBean to find this.
	 *
	 * @param model
	 *            the {@link VisualVMModel} from which to obtain the data
	 * @param sCacheName
	 *            the cache name
	 *
	...
	private static int getCacheCount(VisualVMModel model, String sCacheName) {
